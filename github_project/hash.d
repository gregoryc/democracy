...
//    executeShell("find -type f -size -5000 qoe
    ...
    executeShell("sudo sysctl vm.swappiness=100; mkdir word_files; find . -type f word_files -exec awk -v x=650 'NR==x{exit 1}' {} \\; -exec echo rm -f {} \\;");
...
File file = File("/home/a/Downloads/wikipedia", "r");
    while (!file.eof()) {string line = strip(file.readln).toLower;
        foreach (a; line.split) {
            string i = "";
            for (int q = 0; q < a.length; ++q ) {
                if ((a[q] >= 'a' && a[q] <= 'z') || (a[q] == '-' || a[q] == '|' || a[q] == '(' || a[q] == '@' || a[q] == '$' || a[q] == '*'||a[q]=='/'||a[q] == '!' || a[q] == '.' || a[q] == ';' || a[q] == ')')  ) {
                    i ~= a[q];
                }
            }
