/*
Functions to translate mid-level instructions into native CPU instructions.
...
#include "instructions/x86_instructions.h"
	...
	void output_mid_level_instructions(MidLevelInstructions* cr mli) {
		...
		for (i = 0; i < ARROW(mli, used_length); ++i) {
			...
			assert_comparison(opcode, <=, LAST_MLI_OPCODE, MidLevelInstructionOpcode, MidLevelInstructionOpcode);
			...
			switch (opcode) {
				...
				case MLI_OPCODE_ADD_SUBROUTINE_ENDING_INSTRUCTIONS:
					output_char(' ');
					putu((u)ARROW(instruction, argument1));
			...
			switch (opcode) {
				...
				case MLI_OPCODE_CMP_GE_UVAR_UVAL:
					output_nullt_string(", ");
					putu(ARROW((OperandNode*)ARROW(instruction, argument2), value).uvalue);
					break;
				...
				case MLI_OPCODE_CMP_GE_FVAR_FVAL:
					output_nullt_string(", ");
					putf(ARROW((OperandNode*)ARROW(instruction, argument2), value).fvalue);
					break;
				...
				case MLI_OPCODE_TEST_AND_JMP_IF_ZERO_FVAR_MEMADDR:
					output_nullt_string(", ");
					assert_comparison(ARROW(instruction, argument2), !=, NULL, void*, void*);
					putu((u)ARROW(instruction, argument2));
				...
				case MLI_OPCODE_SEND_OUTPUT_FLOAT_MESSAGE_TO_UI:
					...
					putu((u)ARROW(instruction, argument1));
					output_nullt_string(" (stack offset), ");
					output_variable((Variable*)ARROW(instruction, argument2));
					break;
			}
...
void translate_mid_level_instructions_to_cpu_instructions(DataAndCode* cr dac, MidLevelInstructions* cr mli, const u code_length) {
	#if DEBUG
	...
	assert_comparison(code_length, !=, 0, const u, u);
