...
u dac_get_length_of_fcomi(void) {
	return 2;
}

void dac_append_fcomi(DataAndCode* cr dac, const FPU_Register register_) {
	assert_comparison(dac, !=, NULL, DataAndCode*, void*);
	assert_comparison(register_, <=, REGISTER_ST7, FPU_Register, FPU_Register);
	
	#if DEBUG
		output_nullt_string(TAB_STRING TAB_STRING "fcomi st");
		
		/*
		This is a minor optimization.
		putu() can be used instead, but putu_without_separators() is slightly faster.
		*/
		putu_without_separators(register_);
		
		output_newline();
	#endif
	
	ARRAY_INDEX_ASSIGN(ARROW(dac, start_of_code), ARROW(dac, current_instruction_offset)) = (uchar)219;
