	...
	variable_stack_offset = ARROW(variable, stack_offset);
		...
		if (variable_stack_offset) {
			...
			if (variable_stack_offset <= 127) {
				return dac_get_length_of_fstp_deref_register_as_qword_plus_signed_byte_offset(REGISTER_RSP);
			} else {
				return
	...
	for (i = 0; i < ARROW(mli, used_length); ++i) {
		...
		switch (ARROW(instruction, opcode)) {
			...
			case MLI_OPCODE_MOV_FVAR_FVAR: {
				...
				variable1 = (Variable*)ARRAY_INDEX(instruction->arguments, 0);
				variable2 = ARROW((OperandNode*)ARRAY_INDEX(instruction->arguments, 1), variable);
			...
			case MLI_OPCODE_ADD_FVAR_FVAR: {
				...
				variable1 = (Variable*)ARRAY_INDEX(instruction->arguments, 0);
				variable2 = (Variable*)ARRAY_INDEX(instruction->arguments, 1);
			...
			case MLI_OPCODE_CMP_LT_FVAR_FVAL: {
				Variable* variable;
				OperandNode* operand_node;
				u length_of_fld1_and_jmp_short;
				...
				length += dac_get_length_of_fstp_fpu_register();
				
				length_of_fld1_and_jmp_short = dac_get_length_of_fld1() + dac_get_length_of_jmp_short();
				
				length += dac_get_length_of_jbe(length_of_fld1_and_jmp_short);
				length += length_of_fld1_and_jmp_short;
				length += dac_get_length_of_fldz();
			...
			case MLI_OPCODE_CMP_GE_FVAR_FVAL: {
				...
				length += dac_get_length_of_fcomip();
				length += dac_get_length_of_fstp_fpu_register();
				length += dac_get_length_of_jb_short();
				length += dac_get_length_of_fld1();
			...
			case MLI_OPCODE_TEST_AND_JMP_IF_ZERO_FVAR_MEMADDR: {
				...
				if (likely(relative_instruction_offset)) {
					...
					length += dac_get_length_of_fcomip();
					length += dac_get_length_of_fstp_fpu_register();
					length += dac_get_length_of_je(relative_instruction_offset);
				}
			...
			case MLI_OPCODE_SEND_HELLO_WORLD_MESSAGE_TO_UI: {
				const u variable_stack_offset = (u)ARRAY_INDEX(instruction->arguments, 0);
						...
						if (!variable_stack_offset) {
							...
							length += dac_get_length_of_mov_reg64_reg64();
							length += dac_get_length_of_add_reg64_lit32(REGISTER_RSI, (s)ARRAY_INDEX(instruction->arguments, 0));
						} else {
							length += dac_get_length_of_mov_reg64_reg64();
							length += dac_get_length_of_mov_reg64_lit64(REGISTER_R11, (s)ARRAY_INDEX(instruction->arguments, 0) /* stack offset */);
							length += dac_get_length_of_add_reg64_reg64();
						...
						if (!variable_stack_offset) {
							...
							length += dac_get_length_of_mov_reg32_reg32();
							length += dac_get_length_of_add_reg32_lit32(REGISTER_ESI, (s)ARRAY_INDEX(instruction->arguments, 0));
						} else {
							length += dac_get_length_of_mov_reg32_lit32(0 /* the message number */);
							length += dac_get_length_of_mov_reg32_reg32();
							length += dac_get_length_of_mov_reg32_lit32((s)ARRAY_INDEX(instruction->arguments, 0) /* stack offset */);
							length += dac_get_length_of_add_reg32_reg32();
			...
			case MLI_OPCODE_ADD_MAIN_SUBROUTINE_STARTING_INSTRUCTIONS: {
				...
				if (
					unlikely(
						i + 1 < ARROW(mli, used_length) &&
						ARROW(ARRAY_INDEX(ARROW(mli, mid_level_instructions), i + 1), opcode) ==
			...
			case MLI_OPCODE_ADD_SUBROUTINE_STARTING_INSTRUCTIONS: {
				#if SIXTY_FOUR_BIT
					length += dac_get_length_of_push(REGISTER_RBP);
					length += dac_get_length_of_mov_reg64_lit64_ten_byte_encoding();
					length += dac_get_length_of_mov_reg64_reg64();
					length += dac_get_length_of_sub_reg64_reg64();
				#else
			...
			case MLI_OPCODE_ADD_SUBROUTINE_ENDING_INSTRUCTIONS: {
				...
				#if SIXTY_FOUR_BIT
					length += dac_get_length_of_mov_reg64_reg64();
					length += dac_get_length_of_add_reg64_lit32(REGISTER_RBX, 8);
					length += dac_get_length_of_mov_reg64_deref_register_as_qword(REGISTER_RBX);
				#else
					#error "FIXME"
				#endif
				...
				#if SIXTY_FOUR_BIT
					length += dac_get_length_of_mov_reg64_reg64();
					length += dac_get_length_of_pop(REGISTER_RBP);
					length += dac_get_length_of_mov_reg64_lit64(REGISTER_RBX, (number_of_arguments + 1) * 8);
					length += dac_get_length_of_add_reg64_reg64();
				#else
					#error "FIXME"
				#endif
				...
				#else
					length += dac_get_length_of_jmp_reg(REGISTER_EAX);
				#endif
