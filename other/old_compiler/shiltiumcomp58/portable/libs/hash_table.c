...
void hash_table_init(
	HashTable* cr ht, const u number_of_linked_lists, const u max_amount_of_items_in_a_linked_list, const f multiplication_amount
) {
...
attr_nonnull static u hash_string(HashTable* cr ht, const char* cr key, u length_of_key) {
	...
	while (length_of_key--) {
		hash ^= (hash << 5) + (hash >> 2) + ARRAY_INDEX(key, length_of_key);
  }
  
	return hash_u(ht, hash);
}

static u hash_appropriately_depending_on_type_of_key(HashTable* cr ht, const TypeUnion key, const u length_of_key, const HashType type_of_key) {
	assert_comparison(ht, !=, NULL, HashTable*, void*);
...
static void hash_table_insert_for_resize(HashTable* cr ht, const TypeUnion key, const u length_of_key, const HashType type_of_key, void* const value) {
...
static bool hash_table_insert(
	...
	new_head->next_node = ht->linked_lists[hash];
...
hash_table_insert_with_string_key(HashTable* cr ht, char* const key, const u length_of_key, void* const value, const bool replace, void*** cr new_value) {
	TypeUnion tu;
	tu.string_value = key;
...
hash_table_insert_with_u_key(HashTable* cr ht, const u key, void* const value, const bool replace, void*** cr new_value) {
	TypeUnion tu;
	tu.uvalue = key;
...
__attribute__((nonnull (1, 5))) bool /* successfully added or replaced a new key? */
hash_table_insert_with_f_key(HashTable* cr ht, const f key, void* const value, const bool replace, void*** cr new_value) {
	TypeUnion tu;
	tu.fvalue = key;
	return hash_table_insert(ht, tu, HASH_TYPE_FLOATING_POINT_NUMBER, 0, value, replace, new_value);
}

attr_nonnull void** hash_table_get_value_from_string_key(HashTable* cr ht, const char* cr key, const u length_of_key, bool* cr in_hash_table) {
	HashItem* item;
	u hash;
...
void hash_table_delete_string_key(
	...
	while (1) {
		if (
			...
			HashItem* next_node = ARROW(node, next_node);
...
attr_nonnull void** hash_table_get_value_from_f_key(HashTable* cr ht, f key, bool* in_hash_table) {
	...
	for (item = ht->linked_lists[hash]; item; item = item->next_node) {
		if (item->type_of_key == HASH_TYPE_FLOATING_POINT_NUMBER && item->key.fvalue == item->key.fvalue) {
			DEREF_ASSIGN(in_hash_table) = true;
			return &item->value;
		}
	}
