...
void operand_linked_list_del(OperandLinkedList* cr linked_list) {
	OperandNode* node;
	
	assert_comparison(linked_list, !=, NULL, void*, void*);
	
	node = ARROW(linked_list, first_node);
	
	while (node) {
		OperandNode* next_node = ARROW(node, next_node);
...
void operator_linked_list_del(OperatorLinkedList* cr linked_list) {
	OperatorNode* node;
	...
	while (node) {
		OperatorNode* next_node = ARROW(node, next_node);
...
static void get_value(OperandNode* const operand_node, HashTable* variables_in_scope) {
	assert_comparison(operand_node, !=, NULL, void*, void*);
	assert_comparison(variables_in_scope, !=, NULL, HashTable*, void*);
			...
			variable =
				(Variable*)DEREF(
					hash_table_get_value_from_string_key(
						variables_in_scope,
						operand_node->string,
						operand_node->string_length,
						&in_hash_table
					)
				);
			
			if (likely(in_hash_table)) {
...
static void process_parsed_expression(
	...
	HashTable* cr variables_in_scope
) {
	...
	while (operand_node) {
		...
		get_value(operand_node, variables_in_scope);
...
attr_nonnull static void give_syntax_error_if_previous_token_type_is_an_operator(
	...
	if (unlikely(previous_token_type == TOKEN_TYPE_OPERATOR)) {
		...
		output_string(ARROW(previous_token, string).string, ARROW(previous_token, string).length);
		...
		output_nullt_string(" " OPENING_QUOTE);
		output_string(ARROW(token, string).string, ARROW(token, string).length);
...
void parse_expression(
	...
	process_parsed_expression(operators, operands, variables_in_scope);
