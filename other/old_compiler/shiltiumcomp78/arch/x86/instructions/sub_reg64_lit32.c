...
#include "get_length_of_add_or_sub_reg64_lit32.h"
	...
	u dac_get_length_of_sub_reg64_lit32(const Register64 register_, const s32 amount) {
		return dac_get_length_of_add_or_sub_reg64_lit32(register_, amount);
	}
	
	void dac_append_sub_reg64_lit32(DataAndCode* cr dac, const Register64 register_, const s32 amount) {
		assert_comparison(dac, !=, NULL, DataAndCode*, void*);
		assert_comparison(register_, <=, REGISTER_R15, Register64, Register64);
		
		#if DEBUG
			output_nullt_string(TAB_STRING TAB_STRING "sub ");
		...
		dac_append_add_or_sub_reg64_lit32(dac, register_, amount, 232, 45);
	}
#endif
