...
#if SIXTY_FOUR_BIT
	attr_const u dac_get_length_of_fadd_qword_address(void) {
		return 6;
	}
	
	attr_nonnull void dac_append_fadd_qword_address(DataAndCode* cr dac, const s rip_relative_address) {
		assert_comparison(dac, !=, NULL, DataAndCode*, void*);
		
		#if DEBUG
			output_nullt_string(TAB_STRING TAB_STRING "fadd qword[rip ");
		...
		ARRAY_INDEX_ASSIGN(ARROW(dac, start_of_code), ARROW(dac, current_instruction_offset)) = (uchar)220;
