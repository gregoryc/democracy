...
#include "x86_instructions_globals.h"
	...
	attr_nonnull void dac_append_mov_deref_register_as_qword_lit32(DataAndCode* cr dac, const Register64 register_, const u number) {
		...
		ARRAY_INDEX_ASSIGN(ARROW(dac, start_of_code), dac->current_instruction_offset) = 72 + (register_ >= REGISTER_R8);
		ARRAY_INDEX_ASSIGN(ARROW(dac, start_of_code), dac->current_instruction_offset + 1) = (uchar)199;
