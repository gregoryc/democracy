...
attr_nonnull static void create_argument(ParserState* cr state, char* const string, const u length) {
	Variable* argument;
	...
	if (unlikely(!argument)) {
		output_nullt_string("Memory error on line ");
		putu(get_line_number(state));
		output_nullt_string(": unable to allocate memory for temporary variable in block statement expression.\n");
	}
	...
	ARROW_ASSIGN(argument, stack_offset) = 0;
	...
	for (i = 0; i < number_of_tokens; ++i) {
		...
		if (
			...
			if (unlikely(!is_valid_identifier(ARROW(ARRAY_INDEX(tokens, i - 1), string).string, ARROW(ARRAY_INDEX(tokens, i - 1), string).length))) {
				...
				output_string(ARROW(ARRAY_INDEX(tokens, i - 1), string).string, ARROW(ARRAY_INDEX(tokens, i - 1), string).length);
			...
			create_argument(state, new_argument_name, ARROW(ARRAY_INDEX(tokens, i - 1), string).length);
	...
	*/
	...
	if (
		...
		output_nullt_string(
			...
			LANGUAGE_NAME " does not have loops. Use recursion instead."
		);
	} else if (
		...
		output_nullt_string(
			...
			" because " LANGUAGE_NAME " does not have loops. Use recursion instead."
		);
	} else {
		char* char_;
		...
		output_nullt_string(CLOSING_QUOTE " ");
		output_nullt_string(
			"statement in " LANGUAGE_NAME " currently, but this might change "
			"in future versions of the language. Currently the only valid block statements are "
			OPENING_QUOTE "if" CLOSING_QUOTE " and " OPENING_QUOTE "unless" CLOSING_QUOTE "."
		);
	}
	
	output_nullt_string("\n\n");
	output_source_lines(statement, get_line_number(state));
...
static Variable* allocate_temporary_variable_in_block_statement_expression(ParserState* cr state, Statement* const statement) {
	...
	static void show_tokens_in_statement(Statement* cr statement) {
		...
		for (i = 0; i < ARROW(statement, length_of_tokens); ++i) {
			...
			output_nullt_string("    ");
			output_string(ARROW(current_token, string).string, ARROW(current_token, string).length);
			output_newline();
		}
	}
#endif
...
static void parse_sub_statement(ParserState* cr state, Statement* cr statement) {
	...
	if (!is_valid_identifier(subroutine_name, subroutine_name_length)) {
		...
		output_string(subroutine_name, subroutine_name_length);
	...
	if (!hash_table_insert_with_string_key(
		...
		output_string(ARROW(ARRAY_INDEX(ARROW(statement, tokens), 0), string).string, ARROW(ARRAY_INDEX(ARROW(statement, tokens), 0), string).length);
	...
	subroutine_forward_reference =
		(SubroutineForwardReference*)DEREF(
	...
	if (in_hash_table) {
		...
		mli_list_head = ARROW(subroutine_forward_reference, mid_level_instruction_list);
	
		while (mli_list_head) {
			LinkedListNode* next_node;
		
			ARROW_ASSIGN((MidLevelInstruction*)ARROW(mli_list_head, value), argument1) = (void*)ARROW(subroutine, code_offset);
		...
		argument_type_list_head = ARROW(subroutine_forward_reference, argument_type_list);
	
		ARROW_ASSIGN(subroutine, argument_type_list) = argument_type_list_head;
		ARROW_ASSIGN(subroutine, length_of_argument_type_list) = ARROW(subroutine_forward_reference, length_of_argument_type_list);
	
		m_free(subroutine_forward_reference);
	...
	if (strnequal(subroutine_name, "main", subroutine_name_length, 4)) {
		...
		if (unlikely(!mid_level_instruction)) {
			output_nullt_string(
				"Unable to allocate memory for a " OPENING_QUOTE "add_main_subroutine_starting_instructions" CLOSING_QUOTE " mid-level instruction.\n"
			);
		}
		...
		if (unlikely(!mid_level_instruction)) {
			output_nullt_string(
				"Unable to allocate memory for a " OPENING_QUOTE "add_subroutine_starting_instructions" CLOSING_QUOTE " mid-level instruction.\n"
			);
		}
	...
	{
		ParserState* new_state = (ParserState*)m_alloc(sizeof(ParserState));
		
		if (unlikely(!new_state)) {
			output_nullt_string("Unable to allocate memory for a ParserState structure.\n");
			exit_program();
		}
		...
		/* This call creates the contents of the block statement. */
		parse_block(new_state);
		
		hash_table_del_with_keys_and_values(&new_state->scope.subroutines_in_scope, subroutine_value_deletion_function);
		m_free(new_state);
	}
	...
	if (main_subroutine) {
		...
		ARROW_ASSIGN(mid_level_instruction, argument1) = (void*)DEREF(ARROW(state, stack_offset));
		ARROW_ASSIGN(mid_level_instruction, argument2) = (void*)ARROW(subroutine, length_of_argument_type_list);
	}
...
static void parse_if_or_unless_statement(ParserState* cr state, Statement* cr statement) {
	...
	
	{
		...
		ARROW_ASSIGN(mid_level_instruction, argument2) = (void*)length;
	}
}
...
attr_nonnull void parse_block_statement(ParserState* cr state) {
	...
	if (unlikely(!statement)) {
		output_nullt_string("Memory error: unable to allocate memory to parse a block statement.\n");
		exit_program();
	}
