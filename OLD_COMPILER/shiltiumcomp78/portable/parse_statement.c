...
static void create_variables_from_argument_expressions_and_push_them_onto_the_stack(
	...
	SubroutineForwardReferences* cr subroutine_forward_reference,
	Token** const tokens,
...
void parse_statement(ParserState* cr state, Statement* cr statement) {
	...
	tokens = ARROW(statement, tokens);
		...
		for (j = 0; j < number_of_tokens; ++j) {
			output_nullt_string("    ");
			output_string(&ARRAY_INDEX(tokens, j)->string);
			output_newline();
		}
	#endif
	...
	if (number_of_tokens >= 4 && ARROW(ARRAY_INDEX(tokens, 2), string).length == 1 && ARROW(ARRAY_INDEX(tokens, 2), string).string[0] == '=') {
		...
		if (unlikely(!is_valid_identifier(ARROW(ARRAY_INDEX(tokens, 1), string).string, ARROW(ARRAY_INDEX(tokens, 1), string).length))) {
			...
			output_string(&ARRAY_INDEX(tokens, 1)->string);
		...
		ARROW_ASSIGN(mid_level_instruction, arguments)[1] = (void*)variable;
	...
	/* This branch handles calling subroutines. */
		...
		if (subroutine) {
			...
			ARROW_ASSIGN(mid_level_instruction, arguments)[0] = (void*)ARROW(subroutine, code_offset);
		} else {
			...
			subroutine_forward_references = (SubroutineForwardReferences*)DEREF(new_value);
			
			if (!subroutine_forward_references) {
				/*
				m_zero_initialized_alloc() is called because all of the members of
				the SubroutineForwardReferences struct should be initialized to zero.
				*/
				subroutine_forward_references = (SubroutineForwardReferences*)m_zero_initialized_alloc(sizeof(SubroutineForwardReferences));
				
				if (unlikely(!subroutine_forward_references)) {
					output_nullt_string("Memory error: unable to allocate memory for a SubroutineForwardReference\n");
					exit_program();
				}
				
				DEREF_ASSIGN(new_value) = subroutine_forward_references;
			}
			...
			linked_list_append(&subroutine_forward_references->mid_level_instruction_list, mid_level_instruction);
		}
		...
		number_of_tokens == 2 &&
		strnequal(ARROW(ARRAY_INDEX(tokens, 0), string).string, "eval", ARROW(ARRAY_INDEX(tokens, 0), string).length, 4)
	) {
		Variable* variable = get_variable(state, &ARRAY_INDEX(tokens, 1)->string);
