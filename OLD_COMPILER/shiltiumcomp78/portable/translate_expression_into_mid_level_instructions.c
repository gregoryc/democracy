...
attr_nonnull
static void create_mov_instruction_for_first_node(
	...
	switch (ARROW(operand_node, type)) {
		case TYPE_UINT:
			ARROW_ASSIGN(mid_level_instruction, opcode) = MLI_OPCODE_MOV_UVAR_UVAL;
			ARROW_ASSIGN(mid_level_instruction, arguments)[0] = accumulator;
			ARROW_ASSIGN(mid_level_instruction, arguments)[1] = operand_node;
			break;
		case TYPE_SINT:
			output_nullt_string("SINT IN MUL AND DIV\n");
			exit_program();
		case TYPE_FLOAT:
			ARROW_ASSIGN(mid_level_instruction, opcode) = MLI_OPCODE_MOV_FVAR_FVAL;
			ARROW_ASSIGN(mid_level_instruction, arguments)[0] = accumulator;
			ARROW_ASSIGN(mid_level_instruction, arguments)[1] = operand_node;
			break;
		case TYPE_BOOL:
			...
			exit_program();
		case TYPE_VARIABLE:
			if (!ARROW3(operand_node, variable, evaluated)) {
				translate_expression_into_mid_level_instructions(state, ARROW(operand_node, variable));
			}
			
			ARROW_ASSIGN(mid_level_instruction, opcode) = MLI_OPCODE_MOV_FVAR_FVAR;
			ARROW_ASSIGN(mid_level_instruction, arguments)[0] = accumulator;
			ARROW_ASSIGN(mid_level_instruction, arguments)[1] = operand_node;
...
static void create_accumulator_variable(
	...
	ARROW_ASSIGN(DEREF(accumulator_variable), stack_offset) = get_next_stack_offset(ARROW(state, stack_offset));
	ARROW_ASSIGN(DEREF(accumulator_variable), evaluated) = true;
	ARROW_ASSIGN(DEREF(accumulator_variable), is_argument) = false;
	
	create_mov_instruction_for_first_node(state, DEREF(accumulator_variable), operand_node);
}
...
static void create_variable_operand_node(
	...
	ARROW_ASSIGN(operand_node, variable) = accumulator;
	
	/* Link operand_node with the previous node. */
	ARROW_ASSIGN(operand_node, previous_node) = previous_node;
	if (previous_node) {
	...
	ARROW_ASSIGN(operand_node, next_node) = next_node;
	if (next_node) {
...
create_instructions_for_multiplication_and_division_subexpression(
	...
	create_mov_instruction_for_first_node(state, accumulator, ARROW(DEREF(operand_node), previous_node));
	...
	while (
		...
		switch (DEREF(operand_node)->type) {
			case TYPE_UINT:
				...
				ARROW_ASSIGN(mid_level_instruction, arguments)[0] = accumulator;
				ARROW_ASSIGN(mid_level_instruction, arguments)[1] = DEREF(operand_node);
				break;
			case TYPE_SINT:
				output_nullt_string("SINT IN MUL AND DIV\n");
				exit_program();
	...
	ARROW_ASSIGN(accumulator, last_instruction_that_uses_this_variable) =
		mid_level_instructions_get_last_instruction(ARROW(state, mli));
...
static void translate_multiplication_and_division_subexpressions(
	...
	operand_node = ARROW(ARROW(variable, operands).first_node, next_node);
	...
	while (operand_node) {
		if (ARROW(operator_node, operator_) == OPERATOR_MULTIPLICATION || ARROW(operator_node, operator_) == OPERATOR_DIVISION) {
			...
			ARROW_ASSIGN(accumulator_variable, name).length = 0;
			ARROW_ASSIGN(accumulator_variable, line_declared_on) = 0;
			ARROW_ASSIGN(accumulator_variable, evaluated) = true;
			ARROW_ASSIGN(accumulator_variable, is_argument) = false;
			
			ARROW_ASSIGN(accumulator_variable, stack_offset) = get_next_stack_offset(ARROW(state, stack_offset));
			
			free_list_append(ARROW(state, free_list_head), accumulator_variable, m_free);
	...
	while (
		...
		switch (DEREF(operand_node)->type) {
			...
			case TYPE_FLOAT:
				...
				break;
			case TYPE_BOOL:
				output_nullt_string("BOOL IN ADD AND SUB\n");
				exit_program();
			case TYPE_VARIABLE:
				...
				ARROW_ASSIGN(mid_level_instruction, arguments)[0] = accumulator;
				ARROW_ASSIGN(mid_level_instruction, arguments)[1] = ARROW(DEREF(operand_node), variable);
		}
...
attr_nonnull
static void translate_comparison_subexpressions(ParserState* cr state, Variable* cr variable) {
	OperandNode* operand_node;
	OperatorNode* operator_node;
	...
	create_mov_instruction_for_first_node(state, variable, operand_node);
	...
	while (operand_node) {
		...
		case TYPE_VARIABLE:
			...
			ARROW_ASSIGN(mid_level_instruction, arguments)[0] = variable;
			ARROW_ASSIGN(mid_level_instruction, arguments)[1] = operand_node->variable;
		...
		previous_operator_node = operator_node;
	...
	ARROW_ASSIGN(variable, last_instruction_that_uses_this_variable) =
		mid_level_instructions_get_last_instruction(ARROW(state, mli));
}
...
static void create_change_sign_mid_level_instruction(
	...
	ARROW_ASSIGN(mid_level_instruction, arguments)[0] = accumulator_variable;
	mid_level_instructions_append(ARROW(state, mli), mid_level_instruction, ARROW(state, dac));
...
attr_nonnull
static void process_unary_operators_for_current_operand(
	ParserState* cr state,
		...
		switch (ARROW(operator_node, operator_)) {
			...
			case OPERATOR_SUBTRACTION:
				if (!accumulator_variable) {
					create_accumulator_variable(state, &accumulator_variable, operand_node);
					assert_comparison(accumulator_variable, !=, NULL, Variable*, void*);
				}
				
				create_change_sign_mid_level_instruction(state, accumulator_variable);
				break;
			case OPERATOR_LESS_THAN:
...
attr_nonnull
static void translate_unary_operators(ParserState* cr state, Variable* cr variable) {
	OperandNode* operand_node;
	OperatorNode* operator_node;
	...
	while (operator_node) {
		if (ARROW(operator_node, is_unary)) {
			process_unary_operators_for_current_operand(state, &operator_node, operand_node, &variable->operands);
			continue;
		} else {
			operand_node = ARROW(operand_node, next_node);
		}
...
attr_nonnull void translate_expression_into_mid_level_instructions(ParserState* cr state, Variable* cr variable) {
	assert_comparison(state, !=, NULL, ParserState* cr, void*);
	assert_comparison(variable, !=, NULL, Variable*, void*);
	
	ARROW_ASSIGN(variable, stack_offset) = get_next_stack_offset(ARROW(state, stack_offset));
	
	translate_unary_operators(state, variable);
	translate_multiplication_and_division_subexpressions(state, variable);
	translate_addition_and_subtraction_subexpressions(state, variable);
	translate_comparison_subexpressions(state, variable);
