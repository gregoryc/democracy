...
attr_nonnull void get_next_char(ParserState* cr state, char utf8_char[7]) {
	...
	if ((char_ >> 5) == 6 /* 6 = 0b110 */) {
		...
		if (unlikely(is_eof(ARROW(state, file)))) {
			output_nullt_string("Unexpected EOF in two-character UTF-8 sequence when trying to read byte 2.\n");
			exit_program();
		}
	...
	if ((char_ >> 4) == 14 /* 14 = 0b1110 */) {
		...
		if (unlikely(is_eof(ARROW(state, file)))) {
			output_nullt_string("Unexpected EOF in two-character UTF-8 sequence when trying to read byte 3.\n");
			exit_program();
		}
	...
	if ((char_ >> 3) == 30 /* 30 = 0b11110 */) {
		...
		if (unlikely(is_eof(ARROW(state, file)))) {
			output_nullt_string("Unexpected EOF in two-character UTF-8 sequence when trying to read byte 4.\n");
			exit_program();
		}
	...
	if ((char_ >> 2) == 62 /* 62 = 0b111110 */) {
		...
		if (unlikely(is_eof(ARROW(state, file)))) {
			output_nullt_string("Unexpected EOF in two-character UTF-8 sequence when trying to read byte 5.\n");
			exit_program();
		}
	...
	if ((char_ >> 1) == 126 /* 126 = 0b1111110 */) {
		...
		if (unlikely(is_eof(ARROW(state, file)))) {
			output_nullt_string("Unexpected EOF in two-character UTF-8 sequence when trying to read byte 6.\n");
			exit_program();
		}
	...
	check_bytes: {
		...
		if (unlikely((char_ << (length + 1)) == 0)) {
			output_nullt_string("The character that was read was detected to not be in the shortest form.\n");
			exit_program();
		}
