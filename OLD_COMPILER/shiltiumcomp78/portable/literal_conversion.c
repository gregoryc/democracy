...
use_exponent_index: {
	...
	for (; i < end; ++i) {
		if (!(string[i] >= '0' && string[i] <= '9')) {
			...
			output_string_with_length(string + i, length_of_exponent_in_string);
...
use_base_index: {
	...
	if (!(
		...
		output_string_with_length(string + i, length_of_base_in_string);
		output_nullt_string(CLOSING_QUOTE "\n");
		exit_program();
	}
...
u convert_literal_to_uint(const char* cr string, u string_length, u line_number) {
	...
	for (i = 0; i < string_length; ++i) {
		...
		if (get_offset(VALID_CHARACTERS_STRING, string[i], &number_this_char_represents)) {
			...
			putu(line_number);
			output_nullt_string(": " OPENING_QUOTE);
			output_string_with_length(string, string_length);
