...
attr_nonnull void open_file_for_reading(File* cr file, const char* cr filename) {
	...
	ARROW_ASSIGN(file, ungot_character) = false;
...
attr_nonnull char get_char_from_file(File* cr file) {
	...
	if (unlikely(ARROW(file, ungot_character))) {
		ARROW_ASSIGN(file, ungot_character) = false;
		return ARROW(file, ungot_char);
	}
	
	if (ARROW(file, offset) == ARROW(file, number_of_bytes_read)) {
		read_some_bytes(file);
	}
...
attr_nonnull void rewind_file(File* cr file, const off_t number_of_bytes) {
	...
	if (unlikely(lseek(file->fd, -number_of_bytes, SEEK_CUR) == -1)) {
	...
	file->offset = file->number_of_bytes_read;
	file->eof = false;
}

attr_nonnull void unget_char(File* cr file, const char char_) {
	assert_comparison(file, !=, NULL, File* cr, void*);
	
	file->ungot_character = true;
	file->ungot_char = char_;
	file->eof = false;
}
