...
#if SIXTY_FOUR_BIT
	attr_const u dac_get_length_of_mov_deref_register_as_qword_plus_signed_byte_offset_lit32(const Register64 register_) {
		return 8 + (register_ == REGISTER_RSP || register_ == REGISTER_R12);
	}
	
	attr_nonnull void dac_append_mov_deref_register_as_qword_plus_signed_byte_offset_lit32(
		DataAndCode* cr dac,
		...
		ARRAY_INDEX_ASSIGN(ARROW(dac, start_of_code), ARROW(dac, current_instruction_offset) + 2) = 64  + register_;
		ARROW_ASSIGN(dac, current_instruction_offset) += 3;
