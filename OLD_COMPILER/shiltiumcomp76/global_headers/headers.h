...
#undef NICE_QUOTES
		...
		#error "Neither __i386__ nor __x86_64__ is defined. " \
			"You must be using some weird architecture, or your version of gcc is messed up."
	#endif
#else
	#define SIXTY_FOUR_BIT  0
#endif
...
/* These are used to reduce the dependency on the C standard library. */
...
#define exit YOU_SHOULD_NOT_USE_EXIT
	...
	#define getc YOU_SHOULD_NOT_USE_GETC
	#define fgetc YOU_SHOULD_NOT_USE_FGETC
	#define ungetc YOU_SHOULD_NOT_USE_UNGETC
#endif
	...
	*/
		...
		#define attrs_cold_and_noreturn
		#define attr_hot
	#else
		#define attr_cold  __attribute__((cold))
		#define attrs_cold_and_nonnull   __attribute__((cold, nonnull))
		#define attrs_cold_and_noreturn  __attribute__((cold, noreturn))
...
/* i18n and l10n are barely done at all. */
typedef enum {
	LANGUAGE_ENGLISH,
	LANGUAGE_FRENCH,
	LANGUAGE_GERMAN,
...
/*
These are mid-level instruction opcodes -- opcodes for an intermediary code representation.
This also definitely helps the compiler's portability, for obvious reasons.
*/
typedef enum {
	/* Assignment */
...
typedef enum {LOOP_HASNT_FINISHED_YET, LOOP_HAS_FINISHED} LoopHasFinished;
...
typedef struct FreeListNode {
	...
	/* The function that frees the 'block_to_free' member. */
...
#include "../portable/libs/hash_table.h"
...
void s_to_string(s number, char* cr string, small_num_type base) attr_nonnull;
...
/* input.c */
void open_file_for_reading(File* cr file, const char* cr filename) attr_nonnull;
char get_char_from_file(File* cr file) attr_nonnull;
void rewind_file(File* cr file, const off_t number_of_bytes) attr_nonnull;
void unget_char(File* cr file, const char char_) attr_nonnull;
bool is_eof(File* cr file) attr_nonnull;
void close_file(File* cr file) attr_nonnull;
...
void linked_list_append_n_bytes(
	LinkedListHead* const head,
	void* const value,
	const u number_of_bytes
...
void output_string_with_length(const char* cr string, const u length) attr_nonnull;
void output_string(String* cr string) attr_nonnull;
...
void output_syntax_error_beginning_text(const u starting_line, const u ending_line) attr_cold;
void output_type_error_beginning_text(const u starting_line, const u ending_line) attr_cold;
void output_comment_warning_beginning_text(const u starting_line, const u ending_line) attr_cold;

void output_memory_error(
	ParserState* cr state,
	const char* cr string
) __attribute__((nonnull (1), cold));

void output_memory_error_if_null(
	const bool value,
	ParserState* cr state,
	const char* cr string
...
void output_char(const char char_) attr_flatten;
...
put_instructions_into_code_section(DataAndCode* cr dac, MidLevelInstructions* cr mli) attrs_nonnull_and_warn_unused_result;
...
void translate_expression_into_mid_level_instructions(ParserState* cr state, Variable* cr variable) attr_nonnull;
